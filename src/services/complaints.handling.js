 ...
 * Update the email blacklist if email is complaints
 */
function complaintsHanding() {
    async.waterfall([
        ...
        if (err && !err.hasOwnProperty('status')) {
            log.enterErrorLog(2000, errSource, 'complaintsHanding', 'Failed to get the sqs email information', 'Failed to fetch the sqs data from Amazon', err);
        }
        ...
        setTimeout(function() {
            process.nextTick(complaintsHanding);
        }, 3000);
    });
}
/**
...
function saveResponse(emailFeedback, callback) {
    statusDB.updateEmailStatus(emailFeedback, function(err, status) {
        ...
        if (err) {
            log.enterErrorLog(2001, errSource, 'updateEmailStatus', 'Failed to update the mongo email information', 'Failed to update the collection to mongo', err);
            return callback(err, null);
        }
        blacklistDB.saveEmailBlacklist(emailFeedback.email, FEEDBACK_TYPE, function(err, status) {
            if (err) {
                log.enterErrorLog(2002, errSource, 'saveEmailStatusRecord', 'Failed to get the mongo email information', 'Failed to fetch the collection from mongo', err);
                return callback(err, null);
            }
